...
<template>
  <div class="max-w-3xl mx-auto px-4 py-10">
    ...
    <div v-if="loading" class="text-gray-400">Ladataan...</div>

    <div v-else-if="stories.length === 0" class="text-gray-400 italic">Ei tarinoita vielä.</div>
    ...
    <div v-else class="space-y-8">
      <article v-for="s in stories" :key="s._id" class="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
        ...
        <p class="text-xs text-gray-400 mb-4">{{ s.author }} · {{ formatDate(s.createdAt) }}</p>

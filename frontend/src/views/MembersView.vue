...
// Long-field expand toggle (favDrink, etc.)
const expandedFields = reactive<Record<string, Set<string>>>({});
function isExpanded(memberId: string, field: string) {
  return expandedFields[memberId]?.has(field) ?? false;
}
function toggleField(memberId: string, field: string) {
  if (!expandedFields[memberId]) expandedFields[memberId] = new Set();
  if (expandedFields[memberId].has(field)) expandedFields[memberId].delete(field);
  else expandedFields[memberId].add(field);
}
const TRUNCATE_LEN = 35;
function truncated(val: string) { return val.length > TRUNCATE_LEN ? val.slice(0, TRUNCATE_LEN) + '\u2026' : val; }
function needsTruncate(val: string) { return val.length > TRUNCATE_LEN; }

// Alphabetical grouping
const groupedFiltered = computed(() => {
  const groups: Array<{ letter: string; members: typeof filtered.value }> = [];
  for (const m of filtered.value) {
    const letter = m.name.charAt(0).toUpperCase();
    const last = groups[groups.length - 1];
    if (last && last.letter === letter) last.members.push(m);
    else groups.push({ letter, members: [m] });
  }
  return groups;
});
    ...
    <div v-else class="flex flex-col gap-6">
      <template v-for="group in groupedFiltered" :key="group.letter">
        ...
        <div class="flex items-center gap-3">
          <span class="shrink-0 w-8 h-8 rounded-lg bg-gray-900 border border-gray-800
                       flex items-center justify-center text-sm font-black text-gray-500">
            {{ group.letter }}
          </span>
          <div class="flex-1 h-px bg-gray-800/60" />
        </div>

        <!-- Kortit tässä lohkossa -->
        <div class="flex flex-col gap-4">
          ...
          >
            ...
            <div class="flex-1 min-w-0 p-3 sm:p-5 flex flex-col gap-2 sm:gap-3">
              ...
              <!-- Quote -->
              <p v-if="m.quote" class="text-xs sm:text-sm italic text-dpurple-400/90 leading-relaxed">
                "{{ m.quote }}"
              </p>

              <!-- Meta-tiedot -->
              <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-3 gap-y-1 text-xs sm:text-sm text-gray-400">
                <span v-if="m.born" class="flex items-center gap-1.5">
                  <Cake class="w-3.5 h-3.5 text-gray-500 shrink-0" />{{ m.born }}
                </span>
                <span v-if="m.location" class="flex items-center gap-1.5">
                  <MapPin class="w-3.5 h-3.5 text-gray-500 shrink-0" />{{ m.location }}
                </span>
                <!-- favDrink: truncate long values -->
                <span v-if="m.favDrink" class="flex items-start gap-1.5 relative">
                  <GlassWater class="w-3.5 h-3.5 text-gray-500 shrink-0 mt-0.5" />
                  <span v-if="!needsTruncate(m.favDrink)">{{ m.favDrink }}</span>
                  <span v-else class="flex items-center gap-1 flex-wrap">
                    <span>{{ isExpanded(m._id, 'favDrink') ? m.favDrink : truncated(m.favDrink) }}</span>
                    <button @click.stop="toggleField(m._id, 'favDrink')"
                      class="shrink-0 w-4 h-4 rounded flex items-center justify-center border-0
                             bg-gray-800 hover:bg-gray-700 text-gray-500 hover:text-gray-300
                             text-[10px] leading-none transition-colors font-bold">
                      {{ isExpanded(m._id, 'favDrink') ? '−' : '+' }}
                    </button>
                  </span>
                </span>
                <span v-if="m.highestPromille" class="flex items-center gap-1.5">
                ...
                <a v-if="m.website && m.website.startsWith('http')"
                  ...
                  class="flex items-center gap-1.5 hover:text-dgreen-400 transition-colors">
                  <Globe class="w-3.5 h-3.5 text-gray-500 shrink-0" />{{ m.website.replace(/^https?:\/\//, '') }}
                </a>
                <span v-if="m.email" class="flex items-center gap-1.5">
